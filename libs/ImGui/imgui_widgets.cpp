...
bool ImGui::SliderAngle(const char* label, double* v_rad, double v_degrees_min, double v_degrees_max, const char* format, ImGuiSliderFlags flags)
{
    if (format == NULL)
        format = "%.0f deg";
    double v_deg = (*v_rad) * 360.0f / (2 * IM_PI);
    bool value_changed = SliderDouble(label, &v_deg, v_degrees_min, v_degrees_max, format, flags);
    if (value_changed)
        *v_rad = v_deg * (2 * IM_PI) / 360.0f;
    return value_changed;
}
